 ...
 * 时间:              下午4:04:42
 */
package com.hunglv.office.pg.view;

import java.util.Map;

import com.hunglv.office.common.BackgroundDrawer;
import com.hunglv.office.common.PaintKit;
import com.hunglv.office.common.autoshape.AutoShapeKit;
import com.hunglv.office.common.borders.Line;
import com.hunglv.office.common.picture.PictureKit;
...
import com.hunglv.office.pg.control.PGEditor;
import com.hunglv.office.pg.control.Presentation;
...
//            (int)(rect.x * zoom), (int)(rect.y * zoom), zoom, (int)(rect.width * zoom), (int)(rect.height * zoom), null, chart.getAnimation());
     ...
     * @param zoom
     */
    private void drawTable(Canvas canvas, PGModel pGModel, PGEditor pGEditor, int i, TableShape tableShape, float zoom, Map<Integer, Map<Integer, IAnimation>> map)
    {
        int i2;
        canvas.save();
        processRotation(canvas, tableShape, zoom);
        ...
        for (int i3 = 0; i3 < cellCount; i3++) {
            ...
            if (cell != null) {
                ...
                this.brRect.set(Math.round(bounds.getX() * zoom), Math.round(bounds.getY() * zoom), Math.round((bounds.getX() + bounds.getWidth()) * zoom), Math.round((bounds.getY() + bounds.getHeight()) * zoom));
                BackgroundDrawer.drawBackground(canvas, pGEditor.getControl(), i, cell.getBackgroundAndFill(), this.brRect, (IAnimation) null, zoom);
                drawCellBorder(canvas, cell, bounds, zoom);
                if (cell.getText() != null) {
                    drawTextShape(canvas, pGModel, pGEditor, i, cell.getText(), zoom, map);
                }
            }
        }
        if (i2 != 255) {
